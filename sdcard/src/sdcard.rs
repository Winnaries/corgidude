...
use k210_hal::spi::{Aitm, FrameFormat, Spi, Spi01, Tmod, WorkMode};
use k210_hal::{
    dmac::{Dmac, DmacChannel},
    gpiohs::GpiohsAccess,
};
...
impl<'a, X: Spi01> SDCard<'a, X> {
    ...
    fn CS_HIGH(&self) {
        GPIOHS::set_output_value(self.cs_gpionum as usize, true);
    }

    fn CS_LOW(&self) {
        GPIOHS::set_output_value(self.cs_gpionum as usize, false);
    }

    fn HIGH_SPEED_ENABLE(&mut self, clock: &Clocks) {
        self.spi.set_clk_rate(10000000.hz(), &clock);
    }

    fn lowlevel_init(&mut self, clock: &Clocks) {
        GPIOHS::set_output_en(self.cs_gpionum as usize, true);
        GPIOHS::set_input_en(self.cs_gpionum as usize, false);
        self.spi.set_slave_select(Some(self.spi_cs));
        self.spi.set_clk_rate(200000.hz(), &clock);
    }
    ...
    fn write_data(&mut self, data: &[u8]) {
        ...
        self.spi.try_send(data).unwrap();
    }

    fn write_data_dma(&mut self, data: &[u32]) {
        self.spi.configure(
            WorkMode::MODE0,
            FrameFormat::STANDARD,
            8, /* data bits */
        ...
        self.spi.send_data_dma(self.dmac, self.channel, data);
    }
    ...
    fn read_data(&mut self, data: &mut [u8]) {
        ...
        self.spi
            .recv_data_dma(self.dmac, self.channel, data)
            .unwrap();
    }

    fn read_data_dma(&mut self, data: &mut [u32]) {
        self.spi.configure(
            WorkMode::MODE0,
            FrameFormat::STANDARD,
            8, /* data bits */
     ...
     */
    fn get_csdregister(&mut self) -> Result<SD_CSD, ()> {
     ...
     *         - `Ok(info)`: Sequence succeed
     */
    fn get_cidregister(&mut self) -> Result<SD_CID, ()> {
     ...
     *         - `Ok(info)`: Sequence succeed
     */
    fn get_cardinfo(&mut self) -> Result<SD_CardInfo, ()> {
     ...
     */
    pub fn init(&mut self, clock: &Clocks) -> Result<SD_CardInfo, InitError> {
        /* Initialize SD_SPI */
        self.lowlevel_init(&clock);
        /* SD chip select high */
        self.CS_HIGH();
